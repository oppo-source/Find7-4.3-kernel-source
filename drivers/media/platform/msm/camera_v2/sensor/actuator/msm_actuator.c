...
/*Added by Jinshui.Liu@Camera 20140821 start for avoid actuator error crash*/
#ifdef CONFIG_VENDOR_EDIT
    if (!a_ctrl->step_position_table || a_ctrl->curr_step_pos<0 || a_ctrl->curr_step_pos>a_ctrl->total_steps) {
		pr_err("position error!curr_step_pos %d, total_steps %d\n",
		    a_ctrl->curr_step_pos, a_ctrl->total_steps);
		return  -EFAULT;
    }
#endif
/*Added by Jinshui.Liu@Camera 20140821 end*/
